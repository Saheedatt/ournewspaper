...
function Women() {
  ...
  useEffect(() => {
    const fetchWomenNews = async () => {
      try {
        const WomenNews = await fetchNewsByCategory("Women", page);
        setNews(WomenNews);
      } catch (error) {
        console.error("Error fetching Women news:", error);
      }
    };
    fetchWomenNews();
  }, [page]);
  ...
  return (
    <section className="mb-10 px-10">
      <h2 className="text-center uppercase mb-6 text-2xl"> Women News</h2>
      <ul>
